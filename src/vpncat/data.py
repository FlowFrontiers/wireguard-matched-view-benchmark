...
from vpncat.matched_packets import SessionReconstruction, load_flow_table, reconstruct_session
...
SPLIT_FILENAME = "split_manifest.csv"
...
def _canonical_session(
    ...
    *,
    ...
    feature_columns = [column for column in CANONICAL_COLUMNS if column not in result]
...
def build_canonical_dataset(config: DatasetConfig, *, force: bool = False) -> dict[str, Any]:
    """Rebuild matched-view statistics and sequences from public packet pairs."""
    config.output_dir.mkdir(parents=True, exist_ok=True)
    canonical_path = config.output_dir / CANONICAL_FILENAME
    split_path = config.output_dir / SPLIT_FILENAME
    audit_path = config.output_dir / ASSIGNMENT_AUDIT_FILENAME
    manifest_path = config.output_dir / MANIFEST_FILENAME
    outputs = (canonical_path, split_path, audit_path, manifest_path)
    ...
    work_root = config.output_dir / ".m11_work"
    if work_root.exists():
    ...
    try:
        for session in sorted(config.flow_files):
            ...
            class_support.update(
                flows.loc[eligible, "application_category_name"].astype(str)
            )
            ...
            reconstructions[session] = reconstruct_session(
                ...
                maximum_length=config.maximum_prefix_length,
        ...
        retained_classes = {
            label
            for label, support in class_support.items()
        ...
        canonical = canonical.sort_values(["session", "source_id"], ignore_index=True)
        pair_index = canonical.loc[:, ["pair_id", "session", "application_category"]]
        ...
        temporary_split = split_path.with_suffix(".csv.tmp")
    ...
    directional_bytes_checked_by_session = {
        session: int(
            reconstruction.assignment_audit[
                "directional_bytes_fidelity_checked"
            ].sum()
        )
        for session, reconstruction in reconstructions.items()
    }
    directional_bytes_equal_by_session = {
        session: int(
            (
                reconstruction.assignment_audit[
                    "directional_bytes_fidelity_checked"
    ...
    payload: dict[str, Any] = {
        ...
        "input_files": _input_hashes(config),
        "configuration": {
        ...
        "counts": {
            ...
            "endpoint_orientation_checked_packets_by_session": packet_rows_by_session,
            "endpoint_orientation_equal_packets": sum(packet_rows_by_session.values()),
            "endpoint_orientation_equal_packets_by_session": packet_rows_by_session,
            "endpoint_orientation_rate": 1.0,
            "directional_bytes_fidelity_checked_flows": sum(
                directional_bytes_checked_by_session.values()
            ),
            "directional_bytes_fidelity_equal_flows": sum(
                directional_bytes_equal_by_session.values()
            ),
            "directional_bytes_fidelity_rate": 1.0,
            ...
            "dropped_rows": sum(input_rows.values()) - retained_rows,
